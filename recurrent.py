...
class GNSNP(Recurrent):
    ...
    def __init__(self, units,
                 ...
                 **kwargs):
        super(GNSNP, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
    ...
    def build(self, input_shape):
        ...
        self.kernel_o = self.kernel[:, self.units: self.units * 2]
        ...
        if self.use_bias:
            ...
            self.bias_o = self.bias[self.units: self.units * 2]
    ...
    def preprocess_input(self, inputs, training=None):
        if self.implementation == 0:
            ...
            x_a = _time_distributed_dense(inputs, self.kernel_a, self.bias_a,
                                          self.dropout, input_dim, self.units,
                                          timesteps, training=training)
            return K.concatenate([x_r, x_o, x_a], axis=2)
        else:
            return inputs
    ...
    def step(self, inputs, states):
        ...
        if self.implementation == 2:
            ...
            x_o = matrix_x[:, self.units: 2 * self.units]
            x_a = matrix_x[:, 2 * self.units:]
            recurrent_r = matrix_inner[:, :self.units]
            recurrent_o = matrix_inner[:, self.units: 2 * self.units]
            recurrent_a = matrix_inner[:, 2 * self.units:]

            r = self.recurrent_activation(x_r + recurrent_r)
            o = self.recurrent_activation(x_o + recurrent_o)
            ...
            if self.implementation == 0:
                ...
                x_o = inputs[:, self.units: 2 * self.units]
                x_a = inputs[:, 2 * self.units:]
            elif self.implementation == 1:
                x_r = K.dot(inputs * dp_mask[0], self.kernel_r)
                x_o = K.dot(inputs * dp_mask[1], self.kernel_o)
            ...
            r = self.recurrent_activation(x_r + K.dot(h_tm1 * rec_dp_mask[0],
                                          self.recurrent_kernel_r))
            o = self.recurrent_activation(x_o + K.dot(h_tm1 * rec_dp_mask[1],
                                          self.recurrent_kernel_o))
            a = self.activation(x_a + K.dot(r * h_tm1 * rec_dp_mask[2],
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(GNSNP, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
