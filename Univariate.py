...
for i in range(30):
	dataset_name="CNYUSD"
	# Download Data
	series = read_csv('CNYUSD.csv', header=0, parse_dates=[0], index_col=0, squeeze=True,
					  date_parser=parser)
	...
	train, test = supervised_values[0:-396], supervised_values[-396:]
...
# Construct an LSTM network model and train it, the number of samples: 1, the number of cyclic training: 100, the number of neurons in the GSNP layer is 8
	...
	rmse = sqrt(mean_squared_error(raw_values[-396:], predictions))
	mse = mean_squared_error(raw_values[-396:], predictions)
	meanV = np.mean(raw_values[-396:])
	error = abs(raw_values[-396:] - predictions)
	mae = mean_absolute_error(raw_values[-396:], predictions)
	mape = mean_absolute_percentage_error(raw_values[-396:], predictions)

	print(dataset_name + 'Test RMSE: %.10f,MSE:%.10f,MAE:%.10f,MAPE:%.10f' % (rmse, mse, mae, mape))

	np.savetxt('./GNSNP/CNYUSD/prediction/' + str(count) + '_cnyusd.csv', predictions, delimiter=',')
	np.savetxt('./GNSNP/CNYUSD/origin/' + str(count) + '_cnyusd.csv', raw_values[-396:], delimiter=',')
